...
class ConnectionWorker(QObject):
    """Рабочий класс для асинхронных операций подключения через gRPC"""
    ...
    def __init__(self, connection_manager):
        """
        Args:
            connection_manager: GRPCConnectionManager для управления подключением
        """
        super().__init__()
    ...
    def check_remote_connection(self, host: str, port: int):
        """
        Синхронная обертка для асинхронной проверки подключения.
    ...
    async def _check_remote_connection_async(self, host: str, port: int):
        """Асинхронная проверка подключения к удалённому серверу через gRPC"""
        ...
        try:
            ...
            if success:
                ...
                self.server_version.emit(f"SpeechEQ Server (gRPC) v1.0.0")
    ...
    def disconnect_from_server(self):
        """Синхронная обертка для отключения от сервера"""
...
class ConnectionScreenLogic:
    ...
    def on_mode_changed(self, index):
        ...
        # Если переключились на локальный режим, отключаемся от сервера
    ...
    def on_start_local_clicked(self):
        ...
        if self.is_local_running:
            ...
            # Запускаем локальный сервер (заглушка)
